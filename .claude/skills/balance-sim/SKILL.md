...
`yarn sim` runs AI-vs-AI battles on the real engine (`src/battle/*`) — no rendering.

```
yarn sim            # 2000 random 3v3 battles, normal AI
yarn sim --n 5000   # more samples (±1% noise needs ~5000)
yarn sim --ai hard  # both sides use the hard profile
yarn sim --duel     # 1v1 line-vs-line matrix (row win % vs column)
```
...
`yarn levels:tune [--iters 6 --n 24000 | --only id1,id2]` runs the simulator in parallel worker processes and
...
2. Movesets (keep them FireRed-legal: `yarn roster:validate`).
...
## Procedure
1. Baseline: `yarn sim --n 3000` and save the table.
...
4. Run `yarn test` (engine tests must still pass) and record the final table in the commit message.
