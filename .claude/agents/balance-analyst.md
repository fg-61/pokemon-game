...
Method:
1. Establish a baseline (`yarn sim --n 3000`, and `--duel` when a single line is suspicious).
...
3. Prefer the least invasive fix: line `level` → moveset (must pass `yarn roster:validate`) → `CONFIG` → engine rule.
4. Change one thing at a time; re-run with the same sample size; treat < 2% deltas at n=3000 as noise.
5. Finish with `yarn test` green and report before/after tables plus the reasoning. Don't commit unless asked.
